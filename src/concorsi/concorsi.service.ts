...
export class ConcorsiService {
    ...
    async createOrUpdateFromCrawl(dto: CrawlConcorsoDto): Promise<{ concorso: Concorso, status: CrawlStatus }> {
        ...
        if (existingConcorso) {
            ...
            const desc: string = dto.description || existingConcorso.description;
            ...
            const existingStartDate = new Date(existingConcorso.startDate);
            const existingEndDate = new Date(existingConcorso.endDate);

            if (existingStartDate.toISOString() !== dto.startDate.toISOString()) hasChanges = true;
            if (existingEndDate.toISOString() !== dto.endDate.toISOString()) hasChanges = true;
